...
@Autonomous
public class FullRedSpec extends OpMode {
    ...
    public void loop() {
        ...
        if (scorePreloadPathEnd) {
            ...
            scorePreloadPathEnd = false;
        } else if (loadClipsPathEnd) {
            CommandScheduler.getInstance().schedule(refreshSamples());

            redSpecPath = pickupSpikeMark();

            loadClipsPathEnd = false;
        } else if (pickupSpikeMarkPathEnd) {
            // TODO: add a goToSub path with and without spike mark sample

            pickupSpikeMarkAttempts++;

            if (robot.intake.isSample()) {
                redSpecPath = goToSubWithSample();
            } else if (pickupSpikeMarkAttempts < 2){
                CommandScheduler.getInstance().schedule(refreshSamples());
                redSpecPath = pickupSpikeMark();
            } else {
                redSpecPath = goToSub();
            ...
            redSpecPath = intakingLoop();

            goToSubPathEnd = false;


        } else if (intakeScorePathEnd) {
            if (robot.intake.isSample()) {

                if (backUpEnd && Globals.CLIP_LOADED) {
                    redSpecPath = clipAndScore();
                    backUpEnd = false;
                } else if (!backUpEnd) {
                    redSpecPath = backUp();
                } else if(!Globals.CLIP_LOADED) {
                    redSpecPath = new SequentialAction();
                }

            } else {
                if (initialIntakeScan) {
                    CommandScheduler.getInstance().schedule(refreshSamples());

                    initialIntakeScan = false;
                } else {
                    CommandScheduler.getInstance().schedule(incrementTargetedSample());
                }
